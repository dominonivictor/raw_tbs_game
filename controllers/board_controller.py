...
def create_grid(board, size, t_coords={}, i_spaces=[]):
    """
    TAGS: BOARD, GAME
    """
    board.grid = []
    ...
    if not board.selected_tile:
        ...
        if actor.has_died:
            ...
            targeting_command(
                ...
                actor=actor,
                target_tile=target_tile,
                commandable_tiles=tiles,
            )

        board_clean_selected_things(
            board=board, but_not_selected_tile=True, target_tile=target_tile
        )
...
def targeting_move(actor, current_tile, target_tile, movable_tiles, board):
    ...
    if actor.has_died:
        ...
        command = UIMoveCommand(
            current_tile=current_tile, target_tile=target_tile, board=board, actor=actor
        )
...
def targeting_command(board, actor, target_tile, commandable_tiles):
    ...
    if not target_tile.actor and command.id not in ["multiply", "waterball"]:
        ...
        clean_tiles(
            ((x, y) for x, y in region(board.get_width(), board.get_height())),
            board,
            color="last_color",
        )
    else:
...
def pass_turn(board):
    ...
    for row in board.grid:
        for tile in row:
            ...
            tile.rgba = game.grid[x][y].color

    board.remove_lines()
    game_status = game.check_win_condition()
    if game_status.has_ended:
        finish_game_screen(board, game_status.lost_players)


def finish_game_screen(board, lost_players):
    board.endgame_popup(lost_players)
...
def add_actors_in_starting_positions(board):
    ...
    for i, (x, y) in enumerate(board.i_spaces):
        # fixme: need a way to be mapped according to num of actors
...
def highlight_movable_spaces(actor, start_tile):
    ...
    movable_spaces = calculate_dijkstras(
        start_tile, board, game_grid, calculate_cost_to_move, n_moves
    )
...
def highlight_attackable_spaces(command, start_tile):
    ...
    attackable_spaces = calculate_dijkstras(
        start_tile, board, game_grid, calculate_cost_to_attack, n_moves
    )
...
def calculate_dijkstras(start_tile, board, game_grid, step_cost_function, n_moves):
    ...
    while tile_xy:
        tile_xy = (
            min(open_dict, key=lambda k: open_dict[k]["dist"]) if open_dict else None
        )
        if tile_xy:
            tile_dict = open_dict.pop(tile_xy)
            tile_neighbors = get_tile_neighbors(
                tile_xy=tile_xy,
                max_size=board.grid_size,
                game_grid=game_grid,
...
def clean_all_tiles(board, color=""):
    clean_tiles(region(board.get_width(), board.get_height()), board, color=color)
...
def tile_on_hover(tile):
    ...
    if board.selected_command and board.selected_command.category == "aoe":
        ...
        clean_tiles(high_tiles, board, color="last_color")
